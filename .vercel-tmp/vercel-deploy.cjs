...
function doDeploy(projectPath, options) {
  ...
  log('');
  const vercelBin = path.join(os.homedir(), 'AppData', 'Roaming', 'npm', 'node_modules', 'vercel', 'dist', 'vc.js');
  const vercelCmd = fs.existsSync(vercelBin) ? vercelBin : 'vercel';
  const cmdParts = [vercelCmd];
  ...
  log(`Executing: node ${cmdParts.join(' ')}`);
  log('');
  log('========================================');
  try {
    const args = cmdParts.slice(1);
    const result = spawnSync('node', [vercelCmd, ...args], {
      cwd: projectPath,
      encoding: 'utf8',
