...
export function ChangeForm() {
  ...
  const [changeReason, setChangeReason] = useState(
    isEdit ? "根据最新法规要求，第三章安全检查频率需由每季度一次调整为每月一次，同步更新相关条款表述。" : ""
  );
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {isEdit ? "修改更改内容并重新提交" : "选择需要更改的已发布制度或流程，填写更改原因"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <form className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {/* Selected Standard Info + Change Reason */}
          <div className="p-6 space-y-6">
            {selectedStandard ? (
              <>
                ...
                <div className="border-t border-slate-100 pt-6">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700">更改原因 <span className="text-red-500">*</span></label>
                    <textarea
                      value={changeReason}
                      onChange={(e) => setChangeReason(e.target.value)}
                      rows={4}
                      placeholder="请详细描述更改原因，包括更改的必要性、具体内容、影响范围等..."
              ...
              <p className="text-sm text-slate-400 text-center py-8">
                请点击"选择制度/流程"按钮，从已发布的制度/流程中选择需要更改的对象
              </p>
            )}
          </div>
        </div>
