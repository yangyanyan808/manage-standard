...
export function PublishDetail() {
  ...
  const [publish] = useState({
    ...
    status: "completed" as "draft" | "in_progress" | "completed",
    publisher: "李四",
    publishNote: "该制度已通过评议审核，具备发布条件。正式发布后即日生效，旧版同时废止。",
  });
      ...
      case "in_progress":
        return (
          <span
            ...
            onClick={() => window.open(`https://flow.example.com/form/${publish.id}`, "_blank")}
            title="点击跳转至流程表单"
          >
            进行中
          </span>
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-bold text-slate-900">发布详情</h1>
              {getStatusBadge(publish.status)}
            </div>
            <p className="text-sm text-slate-500 mt-0.5">编号: {publish.id} · 发起时间: {publish.date}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {publish.status === "draft" && (
            <button
              onClick={() => navigate(`/publish/${publish.id}/edit`)}
          ...
          {(publish.status === "in_progress" || publish.status === "completed") && (
            <button
              onClick={() => window.open(`https://flow.example.com/form/${publish.id}`, "_blank")}
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-1">
                <label className="text-xs text-slate-500">发布人</label>
                <p className="text-sm text-slate-700">{publish.publisher}</p>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-slate-700">{publish.date}</p>
              </div>
              <div className="space-y-1">
                <label className="text-xs text-slate-500">发布时间</label>
                <p className="text-sm text-slate-700">{publish.publishDate}</p>
              </div>
            </div>

            <div className="border-t border-slate-100 pt-6">
              <div className="space-y-1">
                <label className="text-xs text-slate-500">发布说明</label>
                <p className="text-sm text-slate-700 bg-slate-50 p-4 rounded-lg border border-slate-100 leading-relaxed">{publish.publishNote}</p>
              </div>
            </div>
          </div>
        </div>
