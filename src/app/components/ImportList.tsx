...
import { Plus, Search, Filter, ChevronLeft, ChevronRight, DownloadCloud, MoreHorizontal, Eye, Edit3, Trash2 } from "lucide-react";
...
export function ImportList() {
  ...
  const allImports: ImportItem[] = [
    { id: "IMP-2026-003", title: "ISO 45001职业健康安全管理体系标准导入", importer: "赵六", status: "in_progress", date: "2026-06-28" },
    { id: "IMP-2026-002", title: "GB/T 19001质量管理体系标准导入", importer: "王五", status: "draft", date: "2026-06-22" },
    { id: "IMP-2026-001", title: "ISO 14001环境管理体系标准导入", importer: "张三", status: "completed", date: "2026-06-15" },
  ];

  const filteredImports = allImports.filter(item => {
  ...
  const handleStatusClick = (item: ImportItem) => {
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">将外部标准文件导入系统，建立标准条目并进行管理</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[600px]">
          ...
          <table className="w-full text-left text-sm whitespace-nowrap min-w-[900px]">
            <thead className="bg-slate-50 text-slate-500 sticky top-0 z-10 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">导入人</th>
                ...
                filteredImports.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 group transition-colors">
                    ...
                    <td
                      ...
                      onClick={() => navigate(`/import/${item.id}`)}
                    >
                      {item.title}
                    </td>
                    <td className="px-6 py-4 text-slate-600">{item.importer}</td>
                      ...
                      {openMenuId === item.id && (
                        <>
                          ...
                          <div className="absolute right-6 top-full mt-1 z-20 bg-white border border-slate-200 rounded-lg shadow-lg py-1 w-32">
                            <button
                              onClick={() => { setOpenMenuId(null); navigate(`/import/${item.id}`); }}
                            ...
                            {item.status === "draft" && (
                              <>
                                <button
                                  onClick={() => { setOpenMenuId(null); navigate(`/import/${item.id}/edit`); }}
                                  className="w-full px-3 py-2 text-sm text-left text-slate-700 hover:bg-slate-50 flex items-center gap-2 transition-colors"
                                >
                                  <Edit3 size={14} /> 编辑
                                </button>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center text-slate-500">
                    <div className="flex flex-col items-center justify-center">
                      <DownloadCloud size={48} className="text-slate-200 mb-4" />
                      <p>未找到符合条件的导入记录</p>
                    </div>
                  </td>
                </tr>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-white flex items-center justify-between">
          <div className="text-sm text-slate-500">
            共 <span className="font-medium text-slate-900">{filteredImports.length}</span> 条记录
          </div>
